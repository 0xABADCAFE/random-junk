...
inline vec3 material_sky_rgb2(cvr3 direction) {
    float32 gradient = 1.0f - direction.z;
    gradient *= gradient;
    gradient *= gradient;
    return vec3_scale(
...
inline vec3 material_sky_rgb3(cvr3 direction) {
    float32 gradient = 1.0f - direction.z;
    gradient *= gradient;
    gradient *= gradient;
    return vec3_scale(
...
inline vec3 material_floor_rgb2(vec3& intersection, float32 lambertian) {
    intersection = vec3_scale(intersection, 0.2f);
    return vec3_scale(
        (
            // Compute check colour based on the position
...
inline vec3 material_floor_rgb3(vec3& intersection, float32 lambertian) {
    intersection = vec3_scale(intersection, 0.2f);
    return vec3_scale(
        (
            // Compute check colour based on the position
...
vec3 sample_nobounce(cvr3 origin, cvr3 direction) {
    ...
    // Hit nothing? Sky shade
    if (material == M_SKY) {
        return material_sky_rgb2(direction);
    }
    ...
    return material_floor_rgb2(intersection, lambertian);
}
...
vec3 sample_first_bounce(cvr3 origin, cvr3 direction) {
    ...
    // Hit nothing? Sky shade
    if (material == M_SKY) {
        return material_sky_rgb3(direction);
    }
    ...
    // Hit the floor plane
    if (material == M_FLOOR) {
        return material_floor_rgb3(intersection, lambertian);
    }
    ...
    for (int y = image_size; y--;) {
        for (int x = image_size; x--;) {
            ...
            if (material != M_SKY) {
                ...
                pixel = vec3_scale(material_sky_rgb2(probe_direction), 64 * 3.5);
            }
