  ...
  // camera direction vectors
    ...
    for (int32 y = image_size; y--;) {
        ...
        for (int32 x = image_size; x--;) {
            ...
            for (int32 ray_count = 4; ray_count--;) {

                std::fprintf(
                    stderr,
                    "\nLOOP 2 y=%d, x=%d, ray_count=%d\n",
                    y, x, ray_count
                );

                std::fprintf(stderr, "delta...\n");
                ...
                //std::fprintf(stderr, "delta     = "); delta.debug(2);

                std::fprintf(stderr, "origin...\n");
                vec3 origin = vec3_add(
                    focal_point,
                    delta
                );

                std::fprintf(stderr, "direction...\n");
